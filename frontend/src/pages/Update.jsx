...
const Update= () => {
  ...
  const { id } = useParams();
  const [postInputs, setPostInputs] = useState({ id: '', title: '', content: '' });
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(`${url}/api/v1/todo/${id}`, {
          headers: {
            Authorization: token,
          },
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (postInputs) {
        await axios.put(`${url}/api/v1/todo`, {
          ...postInputs,
        }, {
          headers: {
            'Authorization': token
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      ...
      setTodoContent(truncatedContent); 
    } catch (error) {
      setTodoContent('Error generating content');
    }
  };
    ...
    <div className="flex h-screen">
      ...
      <div className="flex flex-col justify-center px-8 w-1/2">
        <div className="border-2 border-neutral-300 w-[450px] shadow-2xl rounded-xl p-6 mx-auto">
          <h1 className="text-3xl font-bold text-center text-sky-950 mb-2">Update the Todo</h1>
      ...
      {/* <div className="flex-1 flex justify-center items-center w-1/2 bg-white p-6"> */}
        {/* <div className="max-w-2xl w-[450px] bg-white p-8 rounded-lg shadow-lg">
          ...
          <div className="mt-6 p-4 border border-gray-300 h-[275px] rounded-md bg-gray-50">
            <div className='flex items-center justify-between'>
              <h2 className="text-lg font-semibold text-sky-950 mb-2">Generated Todo:</h2>
              <button
                type="button"
                onClick={handleCopy}
            ...
            <p className="text-sky-950">{todoContent || 'Generated todo content will appear here...'}</p>
          </div>
        </div> */}
